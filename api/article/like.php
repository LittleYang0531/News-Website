<?php
	...
	{
		if (!$conn->query("DELETE FROM LikeData WHERE type='article' and id1=$column and id2=$id and uid=".$_COOKIE['DedeUserId']))
		{
	...
	{
		if (!$conn->query("INSERT INTO LikeData (type,id1,id2,uid,time,useragent) VALUES ('article',$column,$id,".$_COOKIE['DedeUserId'].",".time().",'".$_SERVER['HTTP_USER_AGENT']."')"))
		{
			echo ReturnError(-400,"数据库执行错误");
			exit;
		}
	}
	$result=mysqli_query($conn,"SELECT * FROM LikeData WHERE type='article' and id1=$column and id2=$id");
	if (!$result)
	{
		echo ReturnError(-400,"数据库查询错误");
		exit;
	}
	$result1=mysqli_query($conn,"SELECT * FROM LikeData WHERE type='article' and id1=$column and id2=$id and uid=".$_COOKIE['DedeUserId']);
	if (!$result1)
	{
		echo ReturnError(-400,"数据库查询错误");
		exit;
