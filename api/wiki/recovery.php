<?php
	...
	try {
			...
			<body>
				<div class="Main">
					<div>
						...
						<br>
						<br>
						<p>您修改的词条"'.$wikis['Title'].'"(version:'.$article['version'].')已被审核员恢复</p>
						<p>可能是由于审核员操作失误导致您对词条"'.$wikis['Title'].'"的修改被删除</p>
						<p>为您带来不便,深感抱歉</p>
						<p></p>
						<br>
		...
		$mail->send();
		if (!$conn->query("UPDATE wiki SET banned=0 WHERE id=$wiki and version=$version"))
		{
			echo ReturnError(-400,"请求错误");
			exit;
		}
